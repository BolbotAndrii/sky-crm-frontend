    ...
    <Wrapper>
      <InputWrapper>
        <Select
          ...
          size='small'
          onChange={(value, { item }) =>
            heandleChangePicker(item, [`${item.start_date}|${item.end_date}`])
          }
        >
          {date.map((item, idx) => (
            <Option
      ...
      {period.type === 'day' && (
        <DatePicker
          value={
            period?.start_date
              ? moment(period?.start_date, dateFormat)
              : moment()
          }
      ...
      {period.type === 'year' && (
        <YearPicker
          defaultValue={
            period?.start_date
              ? moment(period?.start_date, yearFormat)
              : undefined
          }
          onChange={(date, string) => {
            heandleChangePicker(
              {
                start_date: date
                  ? moment(date).startOf('year').format(dateFormat)
                  : '',
                end_date: date
                  ? moment(date).endOf('year').format(dateFormat)
                  : '',
                type: period.type,
                label: period.label,
              },
          ...
          value={
            period?.start_date
              ? moment(period?.start_date as dayjs.ConfigType)
              : undefined
          }
          format={yearFormat}
          allowClear={false}
